...
function EditPost() {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/api/v1/posts/${id}`)
      .then(raw => raw.json())
      .then(data => {
        const { title, summary, content } = data.post;
        setPost(prevPost => ({
          ...prevPost,
          title,
          summary,
  ...
  return (
    <form className='max-w-md mx-auto' onSubmit={updatePost}>
      <input
      ...
      />
      <button className='mt-1.5 w-full bg-[#555] text-white border-none rounded-md p-2' type="submit">Update Post</button>
    </form>
  )
}
